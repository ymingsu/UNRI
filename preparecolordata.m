close all;
clear;
% clearvars -except dataname anglelist Panglelist datanamelist pali dni bForceUpdate colorpattern;
% bForceUpdate = true;
...
kind=['color-',colorpattern,'-',angleallstr];%color%mono
...
%         disp(num2str(index))
        ...
        [mask] = maskofcolorpolar(colorpattern,sizeimgallnew(2),sizeimgallnew(3))

        mask = permute(mask,[4,1,2,3]);
        img_mosaicked = squeeze(sum(mask.*imgallnew,[1,4]));
        ...
        for iind =1:4
            ...
            imwrite(squeeze(imgallnew(iind,:,:,:)),a1path);
        end
    end
else
    disp('skip Mosaicked')
