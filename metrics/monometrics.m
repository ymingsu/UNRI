function [metrics_psnr,metrics_ssim,metrics_psnr_stokes,metrics_ssim_stokes] = monometrics(polarlist,polarlistref,anglelist,removeEdgesize,bShowPSNR)
...
%% gen our stokes
Stokes_our = pagemldivide(Amat,permute(polarlist,[3,1,2]));
Stokes_our = permute(Stokes_our,[2,3,1]);
ps_our = (Stokes_our(:,:,2)+1i*Stokes_our(:,:,3))./(Stokes_our(:,:,1)+eps);
%% gen gt stokes
Stokes_gt = pagemldivide(Amat,permute(polarlistref,[3,1,2]));
Stokes_gt = permute(Stokes_gt,[2,3,1]);
...
%     disp(['compare,',num2str(imgcp_ind)])
    mp1 = psnr(polarlistref(:,:,imgcp_ind),polarlist(:,:,imgcp_ind));
    ms1 = ssim(polarlistref(:,:,imgcp_ind),polarlist(:,:,imgcp_ind));
    metrics_psnr(imgcp_ind)=mp1;
    metrics_ssim(imgcp_ind)=ms1;
end
% ['PSNR_',anglestr{1}],['PSNR_',anglestr{2}],['PSNR_',anglestr{3}],['PSNR_',anglestr{4}],...
%     'PSNR_S0','PSNR_S1','PSNR_S2','PSNR_Stokes','PSNR_DOLP','CVPSNR_PS',
